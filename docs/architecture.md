...
- The first implementation is a control-plane/integration CLI, not a native coding agent.
- Harness-specific behavior lives behind adapters.
- Local secrets live in OS keychain, not plain-text config.
- Config mutation commands must support dry-run and backups.
- Real detach/resume behavior depends on backend support and should not be misrepresented as complete before that exists.
- Tenant API endpoints use `https://<team>.s46.dev`.
- Auth is device-code only for now.
- `s46 connect` merges into existing harness config.
- Pi uses `~/.pi/agent/models.json` custom provider configuration with `!s46 token --refresh`.
- Generated session IDs follow `@user/readable-slug-randomsuffix`.
- Sharing is Pi-style: HTML export plus secret gist plus viewer URL.
- Landing prepares a human-reviewable package rather than silently bypassing review.
