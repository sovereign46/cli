...
| OS keychain abstraction | Done | `internal/keyring.Store`; cancellable macOS `security`, Linux `secret-tool`, file backend for tests only |
| Typed API boundary | Done | `internal/api.Client`, timeout-bound `HTTPClient`, `MockClient`, `NewClientFromEnv` |
...
| Codex adapter | Done | `internal/harness/codex`, checksum-guarded managed block |
...
| Dry-run mutation plans | Done | Planned operations and unified-style LCS diffs |
...
| Concurrent mutation protection | Done | advisory `flock` around config/state mutations |
| Session command shell | Done | `internal/session` |
| Secure generated session IDs | Done | `@user/readable-slug-randomhex` |
| Pi-style share | Done | real `gh gist create --public=false` flow, plus deterministic mock backend for tests |
...
| Release scaffolding | Done | `.goreleaser.yml`, `.github/workflows/ci.yml`, Homebrew completion generation, `make audit` |
| Version command | Done | `s46 version` with GoReleaser-injected build vars |
| Doctor/disconnect/use UX | Done | `s46 doctor`, `s46 disconnect`, `s46 use` |
...
- `share` can use a deterministic mock backend in tests with `S46_SHARE_BACKEND=mock`.
- `detach`, `resume`, and `land` persist mocked local state rather than starting remote workers.
- `session land` enriches mock review data with available local Git metadata.
