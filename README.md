...
- `s46 airplane setup` installs llama.cpp, downloads the local GGUF model from the signed S46 registry at `models.s46.dev`, verifies the manifest signature and model checksum, installs the verified gateway release from `sovereign46/api`, and starts `llama-server` with the recommended local coding settings.
