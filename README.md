...
s46 models verify <model-id> [--output <path>] [--backend-model <id>] [--yes]
s46 ask "request"
s46 run "task"
```
...
- `s46 airplane setup` installs llama.cpp, downloads or verifies the local GGUF model from the attested s46 registry at `models.s46.dev`, verifies the `s46-attest` DSSE/in-toto manifest attestation against `/trust/v1/root.json` and pinned Sovereign46 keys in production+strict mode, requires a signed passed audit run for the exact model release, checks the advisory/yank index before install, verifies the model checksum before any model probe or local gateway start, installs the verified gateway release from `sovereign46/api`, and starts `llama-server` with the recommended local coding settings.
...
- `s46 models verify <model-id>` is the standalone model-registry verifier. It defaults to the s46 model cache when `--output` is omitted, verifies the same production+strict `s46-attest` manifest attestation, trust root, signed passed audit run, advisory/yank index, and artifact checksum, then prints the signed audit-run URL for that exact model release. Advisory/yank warnings require `--yes` or an interactive `Y` before download. The API-backed publishing migration will require fresh Sigsum checkpoints with an independent witness quorum; legacy slash IDs remain usable only for re-verified S46-certified releases or are reported as legacy-uncertified.
...
For local gateway development, set `S46_API_REPO=/path/to/api`; `make shell` exposes a sandbox symlink when `../s46-api` exists and builds `./cmd/s46-gateway` from that repo.
