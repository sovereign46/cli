...
- `s46 airplane setup` installs llama.cpp, downloads or verifies the local GGUF model from the attested s46 registry at `models.s46.dev`, verifies the `s46-attest` DSSE/in-toto manifest attestation against `/trust/v1/root.json` and pinned Sovereign46 keys, checks the advisory/yank index before install, verifies the model checksum before any model probe or local gateway start, installs the verified gateway release from `sovereign46/api`, and starts `llama-server` with the recommended local coding settings. Missing Sigsum transparency is accepted as a warning by default; set `S46_ATTEST_PRODUCTION=1` or `S46_MODELS_ATTEST_PRODUCTION=1` to fail closed on missing, invalid, or stale transparency.
...
- `s46 models verify <model-id>` is the standalone model-registry verifier. It defaults to the s46 model cache when `--output` is omitted, verifies the same `s46-attest` manifest attestation, trust root, advisory/yank index, and artifact checksum, then prints the signed audit-run URL for that exact model release. Advisory/yank warnings require `--yes` or an interactive `Y` before download.
