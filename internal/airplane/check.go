...
func (s Service) check(ctx context.Context, assumeVerifiedModel bool) Report {
	...
	report.add(Check{Name: "model-downloaded", OK: modelDownloaded, Required: true, Message: boolMessage(modelDownloaded, s.modelPath(), "model is not verified by a s46-attest manifest attestation and passed audit")})
...
func (s Service) llamacppModelCheck(ctx context.Context, modelDownloaded bool, llamacppRunning bool) (bool, string) {
	if !modelDownloaded {
		return false, "skipped: model is not verified by a s46-attest manifest attestation and passed audit"
	}
	if !llamacppRunning {
