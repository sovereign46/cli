...
func TestAirplaneSetupShowsManualModelInstructionsWhenDownloadIsSkipped(t *testing.T) {
	...
	for _, want := range []string{
		...
		"[s46] Automatic setup verifies the s46-attest manifest attestation, trust root, signed passed audit, advisory index, and model checksum before writing or trusting:",
