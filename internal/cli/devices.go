...
func renderDevices(devices []api.Device) []string {
	...
	for _, device := range devices {
		rows = append(rows, []string{device.ID, device.Name, formatDeviceTime(device.LastSeenAt), displayTableCell(device.LastSeenIP)})
	}
	return output.Table([]string{"ID", "NAME", "LAST SEEN", "IP ADDRESS"}, rows)
}
