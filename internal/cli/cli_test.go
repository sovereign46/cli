...
func TestSessionsListsOnlyCurrentProjectTranscriptsAndShareDefaultsToLatest(t *testing.T) {
	...
	newerID := "019e4ad2-ba3a-71f7-b34a-205e84be280f"
	otherID := "019e4ad2-ba3a-71f7-b34a-205e84be2810"
	...
	if !strings.Contains(sessions, newerID) || !strings.Contains(sessions, olderID) || !strings.Contains(sessions, "pi") || !strings.Contains(sessions, projectRoot) {
		t.Fatalf("sessions did not include current-project transcripts:\n%s", sessions)
	}
	if strings.Contains(sessions, otherID) || strings.Contains(sessions, otherRoot) {
		t.Fatalf("sessions included another project's transcript:\n%s", sessions)
	}
	if strings.Index(sessions, newerID) > strings.Index(sessions, olderID) {
		t.Fatalf("sessions should be newest first within the project:\n%s", sessions)
	}

	share := requireOK(t, run(t, env, "share"))
	if !strings.Contains(share, "sharing latest local session: "+newerID+" · pi · "+projectRoot) {
		t.Fatalf("share did not infer latest current-project local session:\n%s", share)
	}
	if strings.Contains(share, otherID) || strings.Contains(share, otherRoot) {
		t.Fatalf("share inferred another project's session:\n%s", share)
	}
...
func writePiSessionFixture(t *testing.T, path string, sessionID string, cwd string, prompt string) {
	...
	content := strings.Join([]string{
		...
		`{"type":"message","timestamp":"2026-05-21T10:00:02.000Z","message":{"role":"assistant","model":"s46/devstral-small-2-24b","content":[{"type":"text","text":"response"}],"timestamp":"2026-05-21T10:00:02.000Z"}}`,
	}, "\n") + "\n"
