	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return app.withLock(cmd.Context(), func() error {
				...
				preflight, err := app.requireCloudAuthBeforeConnectPrompt(cmd.Context(), req)
				if err != nil {
					return err
				}
				...
				return runConnect(cmd.Context(), app, req, preflight)
			})
		},
	}
...
type connectPreflight struct {
	Config      config.Config
	AccessToken string
}

func runConnect(ctx context.Context, app *app, req connectRequest, preflight connectPreflight) error {
	if err := validateConnectRequest(req); err != nil {
		return err
	}
	...
	team, err := connectTeam(ctx, app, targetMode, existing, req, preflight.AccessToken)
	if err != nil {
		return err
	}
...
func (a *app) requireCloudAuthBeforeConnectPrompt(ctx context.Context, req connectRequest) (connectPreflight, error) {
	cfg, err := a.config.LoadConfig()
	if err != nil {
		return connectPreflight{}, err
	}
	preflight := connectPreflight{Config: cfg}
	...
	preflight.AccessToken, err = a.requireAccessToken(ctx)
...
func connectTeam(ctx context.Context, app *app, mode string, existing config.TeamConfig, req connectRequest, accessToken string) (api.Team, error) {
