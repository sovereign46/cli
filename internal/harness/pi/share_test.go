...
`)
	...
	artifact, ok, err := New().ShareArtifact(context.Background(), harness.ShareRequest{Env: map[string]string{"HOME": home}, Session: api.Session{ID: "019e4ad2", Lane: "EU-OPO"}, TeamName: "acme", User: "tester"})
	if err != nil {
		t.Fatal(err)
	}
	if !ok {
