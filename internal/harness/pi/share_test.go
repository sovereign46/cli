...
`)
	...
	artifact, ok, err := New().ShareArtifact(context.Background(), harness.ShareRequest{Env: map[string]string{"HOME": home}, Session: api.Session{ID: "019e4ad2", Region: "EU-OPO"}, TeamName: "@s46/engineering", User: "tester"})
	if err != nil {
		t.Fatal(err)
	}
	if !ok {
